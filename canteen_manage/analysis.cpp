...
void WriteLog(string log)
{
    ...
    *Return：null
    ************************/
    //日志记录函数，将日志写入log.txt文件
    fstream fout("data/opr_log.txt",ios_base::out|ios_base::app);
    if(!fout.is_open())
    {
        cout<<"日志文件开启错误"<<endl;
        return;
...
void SafeStore(SNode * head)
{
    /************************
    * Input：  汇总以后的数据链表表头
    *Output：写入文件secure.txt
    *Return：null
    ************************/
    ofstream fout("data/secure.txt",ios::out);
    int i=1;
    SNode * p=head->next;
    while(p!=nullptr)
    {
